function [out, info, n_step] = DistanceScore(node1,node2,Tree1,Tree2,param,FullTree1,FullTree2,factor, n_step, info)
...
if ((~isempty(node1.parentIndex)) && (~isempty(node2.parentIndex)))
	n_step.A = n_step.A + 1;
	n_step.B = n_step.B + 1;
	eval(['[out, info, n_step] = DistanceScore(FullTree1.n' num2str(node1.parentIndex)...
...
elseif (isempty(node1.parentIndex) && (~isempty(node2.parentIndex)))
	n_step.B = n_step.B + 1;
	eval(['[out, info, n_step] = DistanceScore(FullTree1.n' num2str(node1.index)...
	 ', FullTree2.n' num2str(node2.parentIndex)  ', Tree1, Tree2,param,FullTree1,FullTree2,factor,n_step, info);']);
	info.B = info.B + param*node2.relativeLen;
elseif (~isempty(node1.parentIndex) && ~isempty(node2.parentIndex))
	n_step.A = n_step.A + 1;
