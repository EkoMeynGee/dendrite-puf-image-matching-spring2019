function  [minDistance,parent_mapping]  = munkres(costMat,sizx,sizy)
...
[Mxx,Myy] = size(costMat);
%   if(sizx>sizy)
%          Mxx = sizy;
%          elseif(sizx<sizy)
%           Mxx = sizx;
%          end   
parent_mapping = zeros(sizx,sizy) ;
% oo=1;
% maxv =max(costMat(:));
% for qq=1:Mxx
%   if  (sum(costMat(qq,:)==maxv)~=Myy)
% 	Mat(oo,:) = costMat(qq,:);
%     lindex(oo) = qq;
%     oo=oo+1;
%     
...
% end    
ss=1;
for pp = 1:length(assignment)
    if(assignment(pp)~=0)
         minDistance(ss) = costMat(pp,assignment(pp));
         parent_mapping(pp,assignment(pp)) = 1;
         ss = ss+1;
    end
    end
